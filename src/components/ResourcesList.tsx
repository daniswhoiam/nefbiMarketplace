  ...
  return (
    <div className="w-full space-y-4">
      {currentItems &&
        ...
        currentItems.map((item, index) => <Card resource={item} key={index} />)}
    </div>
  )
}
...
const ResourcesList = ({
  ...
  const [currentItems, setCurrentItems] = useState<Array<Resource> | undefined>(
    undefined
  )
  ...
  const handlePageClick = (event: any) => {
    // Pagination library puts state on event
